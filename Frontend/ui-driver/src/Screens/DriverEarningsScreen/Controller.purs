...
eval (ChangeTab subView') state = exit $ ChangeDriverEarningsTab subView' state
...
eval (RideSummaryAPIResponseAction ridesSummaryList todaysDate datesList) state = do
  ...
  let dataToFetchAndUpdateFromCache = getEarningsToCache earningListWithMissingDates
      ...
      earningsToCache = DA.reverse (DA.drop (length dataToFetchAndUpdateFromCache - 21 - dayOfWeek) (DA.reverse dataToFetchAndUpdateFromCache))
...
eval (UpdateRidesEver anyRidesAssignedEver) state = continue state{data{anyRidesAssignedEver = true}, props{weekIndex = 3}}
