  ...
  , visibility if state.data.payerVpa == "" && state.data.autoPayStatus == ACTIVE_AUTOPAY then GONE else VISIBLE
      ...
      , color Color.black900
                              ...
                              , arrayList : driverPaymentsArray state
                                }] else [detailsListViewComponent state push {  backgroundColor : Color.blue600
                                , separatorColor : Color.white900
...
showLiveStatsDashboard push state =
  ...
  , visibility if (DS.null state.data.config.liveStatsDashboard.url) then GONE else VISIBLE
  , afterRender
        ( \action -> do
      ...
      , url state.data.config.liveStatsDashboard.url
      ]]
...
profileOptionsLayout state push =
  ...
  where visibilityCondition optionItem = 
          case optionItem.menuOptions of
            ...
            LIVE_STATS_DASHBOARD -> state.data.config.liveStatsDashboard.show && state.data.config.liveStatsDashboard.url /= ""
            _ -> true
              ...
              , textView
                ...
                , color case item.value of
                          ...
                          Just val -> do
                                      ...
                                      let isPaymentView = val == (getString VIEW)
...
driverNoAutoPayArray state =  [{ key : (getString GET_QR_CODE) , value : Nothing , action : UpdateValue ST.PAYMENT , isEditable : false  , keyInfo : false , isRightInfo : true }
  ]
