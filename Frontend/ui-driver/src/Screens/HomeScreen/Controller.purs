...
getBannerConfigs state = 
  (if (state.props.driverStatusSet /= ST.Offline && getValueFromConfig "autoPayBanner") && state.props.autoPayBanner /= ST.NO_SUBSCRIPTION_BANNER && not state.props.rideActionModal 
    then [autpPayBannerCarousel state BannerCarousal] 
    else [])
  <> (if (state.props.showGenderBanner && state.props.driverStatusSet /= ST.Offline && getValueToLocalStore IS_BANNER_ACTIVE == "True" && state.props.autoPayBanner == ST.NO_SUBSCRIPTION_BANNER) then [genderBannerConfig state BannerCarousal] else [])
  <> (if (state.props.driverStatusSet /= ST.Offline && state.props.currentStage == ST.HomeScreen && state.data.config.purpleRideConfig.showPurpleVideos) then [accessbilityBannerConfig state BannerCarousal] else [])