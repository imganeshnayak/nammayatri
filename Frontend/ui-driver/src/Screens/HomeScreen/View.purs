...
statsModel push state =
    ...
    , height WRAP_CONTENT
    , orientation VERTICAL
    , visibility if (DA.any (_ == state.props.currentStage) [RideAccepted, RideStarted, ChatWithCustomer]) && not state.props.isStatsModelExpanded then GONE else VISIBLE
    , gravity CENTER
    , padding $ Padding 16 10 16 10
    ][  if not state.data.config.coinsConfig.enableYatriCoins then
        ...
        , padding $ Padding 16 8 8 8
           ...
           [ width WRAP_CONTENT
           , height WRAP_CONTENT
           , text $ getString TODAYS_EARNINGS
...
expandedStatsModel push state =
  ...
  , background Color.white900
  , stroke $ "1,"<> Color.grey900
  , visibility if state.props.isStatsModelExpanded && state.data.config.coinsConfig.enableYatriCoins then VISIBLE else GONE
  , clickable true
  ][ linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
      , gravity CENTER_VERTICAL
      , onClick push $ const $ ToggleStatsModel
      ][ commonTV push (getString TODAYS_EARNINGS) Color.black700 FontStyle.tags LEFT 0 NoAction true
      , commonTV push ("₹" <> formatCurrencyWithCommas (show state.data.totalEarningsOfDay)) Color.black800 FontStyle.h2 RIGHT 0 NoAction false
      , imageView 
        [ width $ V 12
        , height $ V 12
        , margin $ MarginLeft 5
    ...
    , linearLayout
      ...
      , gravity CENTER_VERTICAL
        ...
        , commonTV push (getString VIEW_MORE) Color.blue900 FontStyle.body3 RIGHT 0 GoToEarningsScreen false 
      ]
  ]
