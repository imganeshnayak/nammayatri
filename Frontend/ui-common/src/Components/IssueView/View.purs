...
renderUpdated :: IssueState -> forall w . PrestoDOM (Effect Unit) w
renderUpdated state = linearLayout
                    ...
                    , gravity CENTER_VERTICAL
                    , visibility (if (state.issue.status  == "RESOLVED") then VISIBLE else GONE)
                    ][ textView $
                        [ width WRAP_CONTENT
                        , height WRAP_CONTENT
                        , gravity CENTER
                        , text state.headingText
...
issueView :: IssueState -> forall w . PrestoDOM (Effect Unit) w
issueView state = textView
