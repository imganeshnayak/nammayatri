...
import Debug (spy)
import Engineering.Helpers.Utils (convertNewlinesToBr)
...
chatView config push =
  ...
  , weight 1.0
  , orientation VERTICAL
  , margin $ if os == "IOS" then MarginBottom 100 else Margin 0 0 0 0
  ] ([ scrollView
      [ height MATCH_PARENT
      , width MATCH_PARENT
      ...
      [ linearLayout
        ...
        , orientation VERTICAL
         ...
         ](mapWithIndex (\index item -> chatComponentView config push item (config.messages !! (index+1)) (if (config.messagesSize /= "-1") then (show index == config.messagesSize ) else (index == (length config.messages - 1))) (config.userConfig.appType)) (config.messages))
...
chatComponentView state push config nextConfig isLastItem userType =
  ...
  , width MATCH_PARENT
  , orientation VERTICAL
  , visibility $ if (config.type == "Text" && (STR.length config.message) == 0 ) then GONE else VISIBLE
  ][
  ...
  , onAnimationEnd (\action ->
     ...
     , gravity (getChatConfig state config.sentBy isLastItem (hasTimeStamp config nextConfig)).gravity
        ...
        _ -> textView
          [ textFromHtml $ convertNewlinesToBr if state.spanParent then config.message else getMessageFromKey config.message state.languageKey
...
hasTimeStamp msgConfig nxtMsgConfig = 
  case nxtMsgConfig of
    Just nxtMsgConfig -> not ((not (STR.null msgConfig.timeStamp) && (not (STR.null nxtMsgConfig.timeStamp)) && (convertUTCtoISC msgConfig.timeStamp "hh:mm A") == (convertUTCtoISC nxtMsgConfig.timeStamp "hh:mm A") && msgConfig.sentBy == nxtMsgConfig.sentBy) || STR.null msgConfig.timeStamp)
