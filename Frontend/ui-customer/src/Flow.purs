    ...
    verifyProfile dummy = do
      ...
      if isNothing (response ^. _firstName) 
        then do
          ...
          modifyScreenState $ HomeScreenStateType $ \homeScreen → homeScreen{data{ disability = Just {tag : tag, id : "", description: ""}, settingSideBar{name =fromMaybe ""  (response ^. _firstName),gender = response ^. _gender, email = response ^. _email}} , props {isBanner = false, showSosBanner = not $ fromMaybe false (response ^. _hasCompletedSafetySetup), enableLocalPoliceSupport = fromMaybe false (response^. _enableLocalPoliceSupport)}}
