...
import JBridge (addMediaFile, clearFocus, clearWaitingTimer, generatePDF, hideKeyboardOnNavigation, lottieAnimationConfig, removeMediaPlayer, renderBase64ImageFile, saveAudioFile, scrollToEnd, startAudioRecording, startLottieProcess, stopAudioRecording, toast, uploadFile, uploadMultiPartData, waitingCountdownTimer)
...
eval (AddImagesModelAction (AddImagesModel.OnClickDelete index)) state = do
  ...
  continueWithCmd state { data { addImagesState { images = images', stateChanged = not (imageIds' == state.data.uploadedImagesIds), imageMediaIds = imageIds' } } } [do
    _ <- forWithIndex images' \i x -> do
      _ <- runEffectFn4 renderBase64ImageFile x.image (getNewIDWithTag "add_image_component_image" <> (show i)) false "CENTER_CROP"
      pure NoAction
    pure NoAction
  ]
...
eval (ImageUploadCallback image imageName imagePath) state = do
  ...
  continueWithCmd state { data { addImagesState { images = images',  isLoading = true } } } [do
    _ <- runEffectFn3 uploadMultiPartData imagePath (EndPoint.uploadFile "") "Image"
    pure NoAction
  ]
...
eval (RecordAudioModelAction (RecordAudioModel.OnClickRecord push)) state = do
   continueWithCmd state { data { recordAudioState { timer = "00 : 00" } } }  [do
    cond <- runEffectFn1 startAudioRecording ""
    if cond 
    then do
  ...
  continueWithCmd state { data { recordAudioState { isUploading = true } } } [do
    ...
    case state.data.recordAudioState.recordedFile of
      Just url -> do
                  ...
                  _ <- runEffectFn3 uploadMultiPartData res (EndPoint.uploadFile "") "Audio"
...
eval (RecordAudioModelAction RecordAudioModel.OnClickClose) state =
  ...
  then
    continueWithCmd state { props { showRecordModel = false }, data { recordAudioState { recordedFile = Nothing, recordingDone = false, isRecording = false, openAddAudioModel = false, isUploading = false, timer = "00 : 00" } } } [do
      _ <- runEffectFn1 removeMediaPlayer ""
      _ <- runEffectFn1 stopAudioRecording ""
      _ <- pure $ clearWaitingTimer state.props.timerId
...
eval (ChatViewActionController (ChatView.SendSuggestion optionName)) state = do
  ...
  case find (\x -> x.option == optionName) state.data.options of
    Just selectedOption -> do
      if (selectedOption.label == "CALL_DRIVER")then continue state { props { showCallDriverModel = true, isPopupModelOpen = true }, data{ selectedOption = Just selectedOption }} 
        else  if (selectedOption.label == "REOPEN_TICKET") then exit $ ReopenIssue state { data{ selectedOption = Just selectedOption ,  options = [], chatConfig {suggestionsList = [], messages = messages'}} }
          else do
            pure $ if (selectedOption.label == "DOWNLOAD_INVOICE") then downloadInvoicePDF state else unit
...
eval (PrimaryEditTextActionController (PrimaryEditText.TextChanged _ text)) state =
  continue state { data { messageToBeSent = if (ST.length text >= 256) then state.data.messageToBeSent else text } }
...
downloadInvoicePDF state = 
  case state.data.selectedRide of
    Just ride -> do
      let initData = IS.initData 
      generatePDF (initData {data {selectedItem = ride}}) "NEW"
    Nothing -> unit