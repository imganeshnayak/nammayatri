...
import PrestoDOM (background, frameLayout, imageView, linearLayout, relativeLayout, textView)
...
screen initialState =
  ...
  , globalEvents:
      [ ( \push -> do
            _ <- push ShowOptions 
            if (not initialState.props.initalizedCallbacks) then do
                _ <- JB.storeCallBackImageUpload push ImageUploadCallback
                _ <- runEffectFn2 JB.storeCallBackUploadMultiPartData push UploadMultiPartDataCallback
                _ <- runEffectFn2 JB.storeKeyBoardCallback push KeyboardCallback
                _ <- push InitializeCallback 
                pure $ pure unit
            else 
                pure $ pure unit
        )
