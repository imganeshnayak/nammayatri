...
rideSelection = do
  ...
  case act of 
    ...
    LoaderOutput  updatedState -> do
      modifyScreenState $ RideSelectionScreenStateType (\_ -> updatedState )
      App.BackT $ App.NoBack <$> (pure $ LOADER_RIDES_OUTPUT updatedState)
    SelectRide    updatedState -> do 
      modifyScreenState $ RideSelectionScreenStateType (\_ -> updatedState )
      App.BackT $ App.BackPoint <$> (pure $ SELECT_RIDE updatedState)
    RefreshScreen updatedState -> do 
      modifyScreenState $ RideSelectionScreenStateType (\_ -> updatedState )
      App.BackT $ App.NoBack <$> (pure $ REFRESH_RIDES updatedState)
