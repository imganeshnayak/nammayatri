...
import Helpers.Utils (validateEmail,strLenWithSpecificCharacters, isParentView, emitTerminateApp, getTime,toStringJSON)
...
eval (BackPressed _ ) state = 
  if state.data.issueListType == REPORTED_ISSUES_MODAL then exit $ GoToHelpAndSupportScreen state {data {issueListType = HELP_AND_SUPPORT_SCREEN_MODAL}}
...
eval (SelectRide selectedCategory) _ = exit $ GoToRideSelectionScreen selectedCategory

eval (OpenChat selectedCategory) _ = exit $ GoToChatScreen selectedCategory
...
eval (APIFailureActionController (ErrorModal.PrimaryButtonActionController PrimaryButton.OnClick)) _ = exit GoBack   

eval (NoRidesActionController (ErrorModal.PrimaryButtonActionController PrimaryButton.OnClick)) _ = exit GoHome
...
eval (FetchIssueListApiCall issueList) state = do
     let apiIssueList = getApiIssueList issueList
         updatedResolvedIssueList = reverse (getUpdatedIssueList ["CLOSED"] apiIssueList)
         updatedOngoingIssueList = reverse (getUpdatedIssueList ["OPEN", "PENDING", "RESOLVED", "REOPENED"] apiIssueList)
...
topicsList state =  (if state.data.config.features.enableSelfServe 
  then 
