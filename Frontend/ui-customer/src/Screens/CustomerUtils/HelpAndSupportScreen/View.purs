  ...
  , globalEvents : [
      ...
      ( \push -> do
        _ <- launchAff_ $ void $ EHC.flowRunner defaultGlobalState $ runExceptT $ runBackT $ do
          when (initialState.data.config.features.enableSelfServe) do
          ...
          , width MATCH_PARENT
            ...
            , orientation VERTICAL
              ...
              , allTopicsView state push (topicsList state)
                ] <> ( if state.data.config.features.enableSelfServe then [
  ...
  linearLayout
    ...
    , orientation VERTICAL
        ...
        , onClick push (case item.categoryLabel of
                    ...
                    "SOS"                 -> (const $ SelectRide item)
                    "FARE_DISCREPANCIES"  -> (const $ SelectRide item)
                    "PAYMENT_RELATED"     -> (const $ SelectRide item)
                    "RIDE_RELATED"        -> (const $ SelectRide item)
                    "ACCOUNT_RELATED"     -> (const $ OpenChat item)
                    "OTHER"               -> (const $ OpenChat item)
