...
import Components.FareBreakupModel as FareBreakupModel
import Components.RequestInfoCard as RequestInfoCard
import Components.RideCompletedCard as RideCompletedCard
import Components.ScheduleRide as ScheduleRide
...
bookARentalButtonConfig state =
  let
    ...
    primaryButtonConfig' = config
      ...
      , prefixImageConfig
        { imageUrl = fetchImage FF_ASSET "ny_ic_timer_clock"
        , height = V 16
        , width = V 21
        , margin = (Margin 17 0 17 0)
        }
...
rentalFareBreakupScreenViewState :: ST.HomeScreenState -> FareBreakupModel.FareBreakupScreenState
rentalFareBreakupScreenViewState state = { appConfig: state.data.config
                                         , rentalData: state.props.rentalData
                                         }
  ...
  let
    ...
    rentalRateCardConfig' =
      config'
        ...
        -- , driverAdditionsImage = "ny_ic_ratecard," <> (getCommonAssetStoreLink FunctionCall) <> "ny_ic_ratecard.png"
        , applicableCharges = if state.data.rateCard.nightCharges && state.data.rateCard.vehicleVariant == "AUTO_RICKSHAW" then (getString NIGHT_TIMES_OF) <> (HU.toStringJSON (state.data.rateCard.nightShiftMultiplier)) <> (getString DAYTIME_CHARGES_APPLIED_AT_NIGHT)
                                 else (getString DAY_TIMES_OF) <> (HU.toStringJSON (state.data.rateCard.nightShiftMultiplier)) <> (getString DAYTIME_CHARGES_APPLICABLE_AT_NIGHT)
        , title = "Rental Package"
        , fareList = case MU.getMerchant FunctionCall of
...
rentalScheduleRideConfig :: ST.HomeScreenState -> ScheduleRide.RentalScheduleState
rentalScheduleRideConfig state =
  let
    config' = ScheduleRide.config
    ...
    , isCancelled = state.props.rentalData.showRideCancelled
    , primaryButton = {
        text : if(state.props.rentalData.showRideCancelled) then "Try Again" else "Okay"
      , cornerRadius : state.data.config.primaryButtonCornerRadius
      , background : state.data.config.primaryBackground
