...
import Components.FareBreakupModel.Controller as RentalFareBreakupController
import Components.RequestInfoCard as RequestInfoCard
import Components.ScheduleRide.Controller as ScheduleRideController
...
data Action = NoAction
            ...
            | RentalScheduleRideAction ScheduleRideController.Action
            ...
            | RentalCancelledRideAction ScheduleRideController.Action

eval :: Action -> HomeScreenState -> Eval Action ScreenOutput HomeScreenState

eval (RentalRateCardAction RateCard.Close) state = continue state {props{rentalData{showRentalPackagePopup = false}}}

eval (RentalRateCardAction RateCard.BackPressed) state = continue state {props{rentalData{showRentalPackagePopup = false}}}

eval (RentalScheduleRideAction (ScheduleRideController.PrimaryButtonActionController PrimaryButtonController.OnClick)) state = do
  _ <- pure $ performHapticFeedback unit
  continue state{props{rentalData{showRideCancelled = true, showScheduleRide = false}}}

eval (RentalCancelledRideAction (ScheduleRideController.PrimaryButtonActionController PrimaryButtonController.OnClick)) state = do
  _ <- pure $ performHapticFeedback unit
  continue state{props{currentStage = SearchLocationModel, rentalData{showRideCancelled = false}}}

eval (RentalCancelledRideAction (ScheduleRideController.SecondaryButtonActionController PrimaryButtonController.OnClick)) state = do
  _ <- pure $ performHapticFeedback unit
  continue state{props{currentStage = HomeScreen, bookingStage = NormalBooking, rentalData{showRideCancelled = false}}}
...
eval ( RentalFareBreakupActionController (RentalFareBreakupController.PrimaryButtonActionController PrimaryButtonController.OnClick)) state = do 
  _ <- pure $ performHapticFeedback unit
  continue state{props{rentalData{showScheduleRide = true}}}
...
eval (SearchLocationModelActionController (SearchLocationModelController.DecreaseRentalPackage)) state = do
...
eval (SearchLocationModelActionController (SearchLocationModelController.IncreaseRentalPackage)) state = do
  let baseDuration = fromMaybe (1) (fromString state.props.rentalData.baseDuration)
      baseDistance = fromMaybe (1) (fromString state.props.rentalData.baseDistance)
...
eval (SearchLocationModelActionController (SearchLocationModelController.RentalConfirmAndBookAction PrimaryButtonController.OnClick)) state = do
  ...
  exit $ RentalFlow RENTAL_FARE_BREAKUP_SCREEN state
