...
import JBridge (addMarker, animateCamera, drawRoute, enableMyLocation, firebaseLogEvent, getCurrentPosition, getHeightFromPercent, isCoordOnPath, isInternetAvailable, removeAllPolylines, removeMarker, requestKeyboardShow, showMap, startLottieProcess, toast, updateRoute, getExtendedPath, generateSessionId, initialWebViewSetUp, stopChatListenerService, startChatListenerService, storeCallBackMessageUpdated, isMockLocation, storeCallBackOpenChatScreen, scrollOnResume, waitingCountdownTimerV2, lottieAnimationConfig, storeKeyBoardCallback, getLayoutBounds, clearChatMessages, startTimerWithTimeV2, addCarousel, updateRouteConfig, addCarouselWithVideoExists, storeCallBackLocateOnMap, storeOnResumeCallback)
            ...
            case initialState.props.currentStage of
              ...
              FindingQuotes -> do
                when ((getValueToLocalStore FINDING_QUOTES_POLLING) == "false") $ do
                  ...
                  _ <- if os == "IOS" then do
                        let id = getNewIDWithTag("findingQuotes")
                        startTimerWithTimeV2 (show initialState.props.searchExpire) "findingQuotes" "1" id push SearchExpireCountDown 
              ...
              SettingPrice -> do
                ...
                when (initialState.props.isRepeatRide && initialState.data.config.estimateAndQuoteConfig.enableOnlyAuto) $ do 
                  if (os == "IOS") then do
                    let id = getNewIDWithTag("repeatRide")
                    startTimerWithTimeV2 (show 5) "repeatRide" "1" id push RepeatRideCountDown
              ...
              RideAccepted -> do
                if ( initialState.data.config.notifyRideConfirmationConfig.notify && any (_ == getValueToLocalStore NOTIFIED_CUSTOMER) ["false" , "__failed" , "(null)"] ) then 
                  if (os == "IOS") then do
                    let id = getNewIDWithTag("notifyCustomer")
                    liftEffect $ startTimerWithTimeV2 (show 5) "notifyCustomer" "1" id push NotifyDriverStatusCountDown
                ...
                if ((getValueToLocalStore DRIVER_ARRIVAL_ACTION) == "TRIGGER_WAITING_ACTION") then do
                  ...
                  waitingCountdownTimerV2 initialState.data.driverInfoCardState.driverArrivalTime "1" id push WaitingTimeAction
                  ...
                  if (os == "IOS") then do
                    let id = getNewIDWithTag("SpecialZoneOTPExpiry")
                    liftEffect $ startTimerWithTimeV2 (show secondsLeft) "SpecialZoneOTPExpiry" "1" id push SpecialZoneOTPExpiryAction
