...
stuckBookingsCancel merchantShortId opCity req = do
  ...
  now <- getCurrentTime
  stuckBookingIds <- B.runInReplica $ QBooking.findStuckBookings merchant.id merchantOpCity.id reqBookingIds now
  stuckRideItems <- B.runInReplica $ QRide.findStuckRideItems merchant.id merchantOpCity.id reqBookingIds now
