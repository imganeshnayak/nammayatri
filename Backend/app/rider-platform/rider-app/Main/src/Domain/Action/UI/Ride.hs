...
editLocation rideId (_, merchantId) req = do
  ...
  let attemptsLeft = fromMaybe 1 ride.allowedEditLocationAttempts
  ...
  whenJust req.origin $ \pickup -> do
    when (ride.status /= SRide.NEW) do
      throwError (InvalidRequest "Customer is not allowed to change pickup as the ride is not NEW")
    lastMapping <- QLM.findLastMapping ride.id.getId 0 >>= fromMaybeM (InternalError "Lastest mapping not found")
    initialLocationForRide <- QL.findById lastMapping.locationId >>= fromMaybeM (InternalError "Location not found")
