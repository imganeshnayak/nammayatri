...
customerCancellationDuesSync ::
  ...
  Flow APISuccess
customerCancellationDuesSync merchantShortId _ req = do
  merchant <- QM.findByShortId merchantShortId >>= fromMaybeM (MerchantDoesNotExist merchantShortId.getShortId)
  void $ CallBPPInternal.customerCancellationDuesSync merchant.driverOfferApiKey merchant.driverOfferBaseUrl merchant.driverOfferMerchantId req.mobileNumber req.countryCode req.cancellationCharges req.disputeChancesUsed req.paymentMadeToDriver
  return Success
