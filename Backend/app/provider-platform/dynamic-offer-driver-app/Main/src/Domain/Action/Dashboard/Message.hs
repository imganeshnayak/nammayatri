...
sendMessage merchantShortId opCity Common.SendMessageRequest {..} = do
  ...
  allDriverIds <- case _type of
    AllEnabled -> B.runInReplica $ QP.findAllDriverIdExceptProvided (merchant.id) (merchantOpCity.id) []
    Include -> readCsv
    Exclude -> do
      driverIds <- readCsv
      B.runInReplica $ QP.findAllDriverIdExceptProvided (merchant.id) (merchantOpCity.id) driverIds
...
messageList merchantShortId opCity mbLimit mbOffset = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCity <- CQMOC.findByMerchantIdAndCity merchant.id opCity >>= fromMaybeM (MerchantOperatingCityNotFound $ "merchantShortId: " <> merchantShortId.getShortId <> " ,city: " <> show opCity)
  messages <- MQuery.findAllWithLimitOffset mbLimit mbOffset merchant.id merchantOpCity.id
