...
endRideTransaction driverId booking ride mbFareParams mbRiderDetailsId newFareParams thresholdConfig = do
  ...
  when (newFareParams.customerCancellationDues > 0) $ do
    riderDetailsId <- mbRiderDetailsId & fromMaybeM (InternalError "Rider Id is Null")
    id <- generateGUID
    ...
    let calDisputeChances = newFareParams.customerCancellationDues `div` thresholdConfig.cancellationFee
    riderDetails <- QRD.findById riderDetailsId >>= fromMaybeM (RiderDetailsNotFound riderDetailsId.getId)
    QRD.updateDisputeChancesUsedAndCancellationDues riderDetailsId (max 0 (riderDetails.disputeChancesUsed - fromIntegral calDisputeChances)) 0 >> QCC.create cancellationCharges
